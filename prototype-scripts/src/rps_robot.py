"""
Dieses Script ist das finale Programm für den Schere Stein Papier Roboter. Es liest den Kamerafeed,
...
Style().configure("CustomPanel.TLabel", borderwidth=BORDER, relief="flat",
                  background="black")
...
panel_gesture.pack(side="bottom", fill="x", expand=False,
                   padx=(PADDING, PADDING), pady=(PADDING, PADDING))
label_title = Label(panel_frame, text="Schere Stein Papier - Roboter",
...
for frame in camera.capture_continuous(rawCapture, format="rgb",
    ...
    highest = -1
    if prediction[0] < THRESHOLD and prediction[1] < THRESHOLD and \
            prediction[2] < THRESHOLD and prediction[3] < THRESHOLD:
